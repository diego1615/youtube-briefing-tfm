...
from src.summarizer import bullet_summary, combine_texts, extract_keywords, summarize_text
...
def news_tab(api_key: str, languages: Tuple[str, ...], max_sentences: int) -> None:
...
def setup_tab() -> None:
    ...
    st.markdown(
        ...
        - El resumen de esta version es local y extractivo. No usa un LLM externo.
        """
    )
    st.code("YOUTUBE_API_KEY=tu_api_key", language="bash")
...
def main() -> None:
    ...
    with tab_news:
        news_tab(api_key, languages, max_sentences)
    with tab_setup:
        setup_tab()
